...
function loi(E, signe, tab)
{
    ...
    this.multiple = function (x, n)
    {
        ...
        {
            y = this.multiple(x, n - 1);
            k = y * this.ensemble.cardinal + x;
            return this.table[k]

        }
    }
    this.oppose = function (x)
    {
        if (x == 0)
            return 0;
        else
            return this.ensemble.cardinal - x;
    }
}
function sym(m, n)
{
...
function Paint2()
{
    ...
    var coin = Random(2);
    n = Random(3);
    var m = Op.multiple(x, n);
    if (coin == 1)
    {
        n = -n;
        m = Op.oppose(m);
    }
    var M = A.listel[m].name;
    textzone1.value = "X=" + X + " "+con1+" " + "n=" + n + "  "+con2+"=" + NEUTRE;
    textzone2.value = "nX= " + M;
}
Paint();
