...
//    }
    ...
    if (X.cardinal == 0)
    {
        MAJ = new set(A.cardinal, 'Maj(X)', A.listnum);
    }
    else
    {
        for (i = 0; i < LN.length; i++)
    ...
    if (X.cardinal == 0)
    {
        MIN = new set(A.cardinal, 'Min(X)', A.listnum);
    }
    else
    {
        for (i = 0; i < LN.length; i++)
        ...
        MIN = new set(Q.length, 'Min(X)', Q);
    }
    textzone2.value = X.ext + "  " + MAJ.ext+ "  " + MIN.ext;
