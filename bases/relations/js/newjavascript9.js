...
function relgraph(C, G)
{
    ...
    this.imdir = function (P)
    {
        ...
        for (k = 0; k < this.graph.length; k++)
        {
            ...
            u = X[0];
            v = X[1];
            if ((P.indexOf(u) !== -1) && (R.indexOf(v) === -1))
            {
                R.push(v);
            }
        }
        for (k = 0; k < R.length; k++)
        {
            L[k] = this.cartes.F.listnum[R[k]];
        }
        return new set(R.length, 'Im(X)', L);
    }
    this.imrec = function (P)
    {
        var X, k, u, v;
        var R = [];
        var L = [];
        for (k = 0; k < this.graph.length; k++)
        {
            ...
            u = X[0];
            v = X[1];
            if ((P.indexOf(v) !== -1) && (R.indexOf(u) === -1))
            {
                R.push(u);
            }
        }
        for (k = 0; k < R.length; k++)
        {
            L[k] = this.cartes.E.listnum[R[k]];
        }
        return new set(R.length, 'ImRec(Y)', L);
    }

    this.imdirstr = function (P)
    {
        var I = this.imdir(P);
        return I.ext;
    }
    this.imrecstr = function (P)
    {
        var IR = this.imrec(P);
        return IR.ext;
    }
...
function reltypestr(rel)
{
    switch (rel)
    {
        case 0:
            return "xPy: x a m\u00eame couleur que y";
        case 1:
            return "xPy: x a m\u00eame forme que y";
        case 2:
            return "xPy: x a un num\u00e9ro de m\u00eame parit\u00e9 que celui de y";
    }
}
function Paint()
{
    ...
    InitCanvas1();
    InitCanvas2();
    InitTextZone(textzone1, "#00FF00");
    InitTextZone(textzone3, "#FF99FF");
    var k1 = 3 + Random(4);
    var A = new set(k1, 'E');
    var k2 = 3 + Random(4);
    var B = new set(k2, 'F');
    var C = new cart(A, B);
    var R = new relgraph(C);
    R.show(c1);
    R.saggital(c2);
    ...
    ;
    var Y = new set(P.length, 'Y', Q);
    textzone3.value = Y.ext + "   " + R.imrecstr(P);
