...
function Paint()
{
    ...
    var L1 = [];
    var i, j, k, h;
    for (i = 0; i < A1.cardinal; i++)
    {
        L1.push(A1.listnum[i]);
    }
    k = 1 + Random(3);
    h = 0
    for (i = 0; i < 21; i++)
    ...
    var A2 = new set(L1.length, 'F', L1);
    var k2 = 3 + Random(4);
    var B = new set(k2, 'G');
    var C1 = new cart(A1, B);
    var C2 = new cart(A2, B);
    ...
    for (i = A1.cardinal; i < A2.cardinal; i++)
    {
        j = Random(B.cardinal);
        G2.push([i, j]);
    }
    var R1 = new relgraph(C1, G1);
    var R2 = new relgraph(C2, G2);
    R1.saggital(c1);
    R2.saggital(c2);
    textzone1.value = A1.ext;
    textzone2.value = A2.ext;
 if (document.body.className=="fr") 
      {    textzone3.value = "Graphe de f de E dans "+B.ext;
    textzone4.value = " Graphe de g de F dans "+B.ext;
    textzone5.value = " f est LA restriction de g \u00e0 E";
    textzone6.value = " g est UNE extension de f \u00e0 F";}
  if (document.body.className=="en")
      {    textzone3.value = "Graph of f from E to "+B.ext;
    textzone4.value = " Graph of g from F to "+B.ext;
    textzone5.value = " f is THE restriction of g \u00e0 E";
    textzone6.value = " g is ONE continuation of f \u00e0 F";
}
}
Paint();
