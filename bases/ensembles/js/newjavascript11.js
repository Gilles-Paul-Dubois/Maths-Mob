...
function diffsym(E, F)
{
    ...
    {
        if (ln2.indexOf(ln1[i]) == -1)
            ln3.push(ln1[i]);
    }
    for (i = 0; i < F.cardinal; i++)
    {
        if (ln1.indexOf(ln2[i]) == -1)
            ln3.push(ln2[i]);
    }
    var n3 = ln3.length;
    var D = new set(n3, 'D');
    D.listnum = ln3;
    for (i = 0; i < D.cardinal; i++)
    {
        D.listel[i] = LAE[D.listnum[i]];
    }
    for (i = 0; i < D.cardinal; i++)
    {
        D.listn[i] = D.listel[i].name;
    }
    D.ext = join(',', D.cardinal, D.listn, D.setname);
    return D;
