...
nameSpace2.ddf0 = function (x)
{
    return -Math.cos(x);
}
nameSpace2.ddf1 = function (x)
{
    return 0.4;
}
nameSpace2.ddf2 = function (x)
{
    var y = 1 + x * x;
    return (-4 * y * y + 16 * x * x * y) / (y * y * y * y);

}
nameSpace2.ddf3 = function (x)
{
    return 0.06 * x + 0.1;
}
nameSpace2.ddf4 = function (x)
{
    var y = 2 + Math.sin(x);
    return (4 * Math.sin(x) * y * y + 8 * y * Math.cos(x) * Math.cos(x)) / (y * y * y * y);

}
nameSpace2.ddf5 = function (x)
{
    return 0.4 * 0.4 * Math.exp(0.4 * x);
}
...
nameSpace2.ddf = function (x)
{
    switch (nameSpace2.choice)
    {
        case 0:
            return nameSpace2.ddf0(x);
        case 1:
            return nameSpace2.ddf1(x);
        case 2:
            return nameSpace2.ddf2(x);
        case 3:
            return nameSpace2.ddf3(x);
        case 4:
            return nameSpace2.ddf4(x);
        case 5:
            return nameSpace2.ddf5(x);
    }
}
...
nameSpace2.approx2 = function (i, x)
{
    var a, b;
    a = nameSpace2.ddf(i);
    b = nameSpace2.df(i) - a * i;
    return a * x + b;
}
...
nameSpace2.moveForward = function () {
    if (nameSpace2.animated)
    {
        ...
        nameSpace2.board.removeObject(nameSpace2.generic1);
        ...
        nameSpace2.generic = nameSpace2.board.create('point', [nameSpace2.x, nameSpace2.f(nameSpace2.x)], {size: 1, withLabel: false})
        nameSpace2.turtle2.moveTo([nameSpace2.x, nameSpace2.df(nameSpace2.x)]);
        nameSpace2.generic1 = nameSpace2.board.create('point', [nameSpace2.x, nameSpace2.df(nameSpace2.x)], {size: 1, withLabel: false})
        ...
        nameSpace2.turtle3.moveTo([nameSpace2.x, nameSpace2.ddf(nameSpace2.x)]);
        nameSpace2.A1 = nameSpace2.board.create('point', [nameSpace2.x + 0.5, nameSpace2.approx2(nameSpace2.x, nameSpace2.x + 0.5)], {visible: false});
        nameSpace2.board.create('line', [nameSpace2.generic1, nameSpace2.A1], {color: 'red', straightFirst: false, straightLast: false, lastArrow: true});
    }
...
nameSpace2.go = function ()
{
    ...
    nameSpace2.turtle3 = nameSpace2.board.create('turtle', [nameSpace2.x, nameSpace2.ddf(nameSpace2.x)], {strokeColor: 'brown'});
    nameSpace2.turtle3.hideTurtle();
    nameSpace2.animated = true;
