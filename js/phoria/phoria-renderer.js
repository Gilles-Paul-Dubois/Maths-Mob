    ...
    Phoria.Util.extend(Phoria.CanvasRenderer, Phoria.Renderer, {
        ...
        renderPoint: function renderPoint(ctx, obj, scene, coord, index)
        {
            ...
            switch (obj.style.shademode)
            {
                ...
                case "lightsource":
                {
                    ...
                    ctx.beginPath();
                    ctx.arc(coord[0], coord[1], 2*w, 0, Phoria.TWOPI, true);
                    ctx.closePath();
                    ctx.fill();
                    break;
                }
        ...
        renderEdge: function renderEdge(ctx, obj, scene, edge)
        {
            ...
            if (obj.style.shademode === "lightsource")
            {
                ...
                // draw an edge
                if (edge.arrow == true)
                    drawLineArrow(ctx, coords[edge.a][0], coords[edge.a][1], coords[edge.b][0], coords[edge.b][1]);
                else
                {
                    ctx.moveTo(coords[edge.a][0], coords[edge.a][1]);
                    ctx.lineTo(coords[edge.b][0], coords[edge.b][1]);
                }
                var Beg = obj.points[edge.a];
                if(Beg.name !=undefined)
                ctx.fillText(Beg.name, coords[edge.a][0] + 3, coords[edge.a][1] - 3);
                var End= obj.points[edge.b];
                if (End.name!=undefined)
                ctx.fillText(End.name, coords[edge.b][0] + 3, coords[edge.b][1] - 3);                
                ctx.closePath();
                ctx.stroke();
            }
            else
        ...
        renderPolygon: function renderPolygon(ctx, obj, scene, poly)
        {
            ...
            if (!obj.style.doublesided &&
                    ((coords[vertices[0]][0] * coords[vertices[1]][1] - coords[vertices[1]][0] * coords[vertices[0]][1]) +
                            ...
                            (coords[vertices[2]][0] * coords[vertices[0]][1] - coords[vertices[0]][0] * coords[vertices[2]][1]) < 0))
                return;
            ...
            for (var n = 0, obj; n < scene.renderlist.length; n++)
            {
                ...
                switch (obj.style.drawmode)
                {
                    case "solid":
                    {
                        ...
                        for (var i = 0; i < obj.polygons.length; i++)
                        {
                            if (this.renderPolygon(null, obj, scene, obj.polygons[i]))
                                rendercount++;
                        }
