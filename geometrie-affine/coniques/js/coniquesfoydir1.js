...
nameSpace1.range1 = document.getElementById("myRange71");
nameSpace1.range1.value = 135;
nameSpace1.range2 = document.getElementById("myRange72");
nameSpace1.range2.value = 45;
nameSpace1.theta = 45;
nameSpace1.phi = 30;
nameSpace1.alpha = 135; // paramètre angulaire du point générique del'ellipse

nameSpace1.alpha2 = 220; 
nameSpace1.changeThetaPhi = function ()
{
    cancelAnimFrame(nameSpace1.requestID);
    nameSpace1.theta = parseFloat(nameSpace1.range2.value);
...
nameSpace1.changeM = function ()
{
    cancelAnimFrame(nameSpace1.requestID);
    nameSpace1.alpha = parseFloat(nameSpace1.range1.value);
    nameSpace1.scene.graph = [];
    nameSpace1.scene.graph.push(Phoria.DistantLight.create({
        direction: {x: 0, y: -1, z: -2}
    }));
...
nameSpace1.scene.perspective.fov = 30;// standard par défaut 
...
nameSpace1.generateAllLines = function ()
{
    ...
    pts.push({x: Mx, y: H, z: Mz, name: 'P'}); // index 7
    pts.push({x: (C*H-1)/S, y: H, z: Mz, name: 'H'}); // index 8
    
    var ir2 = nameSpace1.alpha2 * Phoria.RADIANS;
    var lambda2 = -1 / (2 * T * S * Math.cos(ir2) - 2 * C);
    pts.push({x: 2 * lambda2 * T * Math.cos(ir2), y: 2 * lambda2, z: 2 * lambda2 * T * Math.sin(ir2), name: "M'"}); // index 9
        var M1x, M1y, M1z;
    M1x = pts[9].x;
    M1y = pts[9].y;
    M1z = pts[9].z;
    var k2 = H / M1y;
    pts.push({x: k2 * M1x, y: H, z: k2 * M1z, name: "A'"}); // index 10
    pts.push({x: M1x, y: H, z: M1z, name: "P'"}); // index 11
    pts.push({x: (C*H-1)/S, y: H, z: M1z, name: "H'"}); // index 12
    eds.push({a: 0, b: 1});
    eds.push({a: 3, b: 3});
    eds.push({a: 2, b: 3});
    eds.push({a: 0, b: 3});
