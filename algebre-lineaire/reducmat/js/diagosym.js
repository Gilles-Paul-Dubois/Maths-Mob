...
nameSpace2.NextStep = function ()
{
    ...
    nameSpace2.step++;
    var V = new Vecteur(6, 1, nameSpace2.A.M[nameSpace2.step]);
    var W = V.proj(nameSpace2.B);
    var X = V.minus(W);
    var Y = X.unitaire();
    nameSpace2.B.lignes = 1 + nameSpace2.step;
    nameSpace2.B.M.push(Y.coords);
    //nameSpace2.B.show(nameSpace2.c2);
    var i, j;
    var U = [];
    if (nameSpace2.step == 5)
    {
        for (i = 0; i < 6; i++)
            U.push(new Vecteur(6, 1, nameSpace2.B.M[i]));

        for (i = 0; i < 6; i++)
            for(j=0;j<6;j++)
            nameSpace2.C.M[i][j] = U[i].scalaire(U[j]);
       // nameSpace2.C.show(nameSpace2.c3);
    }
}
function DIA(n, K)
{
    result = new Matrice(n, n, K);
    var i, j;
    for (i = 0; i < n; i++)
        for (j = 0; j < n; j++)
        {
...
nameSpace2.Paint = function ()
{
    ...
    nameSpace2.B = new Matrice(0, 6, 1);
    nameSpace2.B.M = [];
    nameSpace2.B.lignes = 0;
    nameSpace2.B.colonnes = 6;
    nameSpace2.B.M = [];
    nameSpace2.C = new Matrice(6, 6, nameSpace2.choice);
    var i, j;
    for (i = 0; i < 6; i++)
        for (j = 0; j < 6; j++)
            nameSpace2.C.M[i][j] = new Reel(0);

    for(i=0;i<6;i++)
        nameSpace2.NextStep();
    nameSpace2.B.show(nameSpace2.c2);
    nameSpace2.C=nameSpace2.B.transpose();
    nameSpace2.C.show(nameSpace2.c3);
    nameSpace2.D= DIA(nameSpace2.lignes, nameSpace2.choice);;
    //nameSpace2.D.show(nameSpace2.c4);
    nameSpace2.A=nameSpace2.B.produit(nameSpace2.D.produit(nameSpace2.C));
    nameSpace2.A.show(nameSpace2.c1);
    nameSpace2.D=nameSpace2.C.produit(nameSpace2.A.produit(nameSpace2.B));
    nameSpace2.D.show(nameSpace2.c4);
}
nameSpace2.Paint();
