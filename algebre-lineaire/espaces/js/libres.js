...
var A = new Point3D(Random(7), Random(7), Random(7), 'u');
var B = new Point3D(Random(7), Random(7), Random(7), 'v');
coin = Random(3);
var C;
if (coin == 0)
{
    ...
    alpha = 1 + Random(2);
    beta = 1 + Random(2);
    C = new Point3D(alpha * A.x + beta * B.x, alpha * A.y + beta * B.y, alpha * A.z + beta * B.z, 'w');
}
else
    C = new Point3D((A.y * B.z - A.z * B.y) / 3, (A.z * B.x - A.x * B.y) / 3, (A.x * B.y - A.y * B.x) / 3, 'w');
...
var G = new Point3D(A.x + B.x + C.x, A.y + B.y + C.y, A.z + B.z + C.z);
...
volume = new shape([Ox, Oy, Oz, ADGE, OBFC, OADB, CEGF, OAEC, BDGF, u, v, w]);
...
function onButton2()
{
    ...
    volume.listFaces = [Ox, Oy, Oz, ADGE, OBFC, OADB, CEGF, OAEC, BDGF, u, v, w];
   ...
   { function loop()
{
    ...
    if (libre())
        textzone1.value = "(u,v,w) système libre";
    else
        textzone1.value = "(u,v,w) système non libre";
    textzone2.value = A.stringCoords() + " " + B.stringCoords() + " " + C.stringCoords();
    if (runAnimation)
        angle += 2;
}}
else
   { function loop()
{
    ...
    C.markName(ctx, angle, 'white');
    if (libre())
        textzone1.value = "(u,v,w) linearly independent system";
    else
        textzone1.value = "(u,v,w) dependent system";
    textzone2.value = A.stringCoords() + " " + B.stringCoords() + " " + C.stringCoords();
    if (runAnimation)
        angle += 2;
}}
